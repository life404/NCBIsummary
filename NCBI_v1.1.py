...
import urllib.error
#如果报错HTTP Error需要使用urllib.error来解析http错误
...
def get_item(GeneSymbol):
    ...
    try:
        ...
        """
        新发现的问题， 某些时候ENSEMBL对应的GeneSymbol对应唯一物种的唯一ID， 也就是说不需要找GeneID， GeneSymbol可以直接搜索到结果，如果继续寻找ID则会报错， 所以使用try和expect来进行选择
        """
        return term
...
def get_summary(IDs):
    ...
    try:
        ...
        """
        如果一个GeneSymbol没有对应的人类的条目,同样会返回上一个函数返回的term, 但是如果使用url2尝试打开网页，会报错404
        """
        return "There is no summary", "None"
    else:
